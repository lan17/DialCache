...
// test/fixtures/kernel. Each typechecks and every run passes; the check runs
// wherever Quint is present (the model check and the differential lanes) so a
// library change cannot move them unnoticed.
...
async function checkFixture(model, { settings, seed, directory, timeoutMs }) {
  ...
  const passing = /(\d+) passing/.exec(log);
  if (test.status !== 0 || !passing || /failed/.test(log)) throw new CommandFailure(`runs of ${model} failed (exit ${test.status}):\n${log}`, test);
  return { model, runs: Number(passing[1]), log, durationMs: typecheck.durationMs + test.durationMs };
}
