    ...
    for (const [i, step] of steps.entries()) {
      ...
      if (step.action === "advance" && currentSource >= 0 && !decoding &&
        ...
        if (classifier === 1 && o.calls.includes(4)) {
          if (operationClassifier === 1) recorder.credit("explicit-denial-overrides-timeout");
          if (operationClassifier === 3 && steps[0].choice % 4 === 2) recorder.credit("instance-denial-overrides-timeout-default");
        }
      }
      if (rejected && !abandoned.has(step.choice)) {
        const instance = steps[0].choice % 4;
        if (instance === 1 && operationClassifier === 1 && o.calls.some((c, j) => c === 3 && previous.calls[j] === 0)) recorder.credit("operation-denial-overrides-instance-allow");
        if (instance === 3 && operationClassifier === 3 && o.calls.some((c, j) => c === 3 && previous.calls[j] === 0)) recorder.credit("instance-classifier-error-preserves-source");
        recoveryCause = step.action === "rejectTimeout" ? "propagated-timeout" : "source-error";
        if (classifier === 3 && step.action === "rejectLoader" && o.calls.includes(3) && o.loads === previous.loads) recorder.credit("default-denies-ordinary-error");
      }
      ...
      if (o.recovery.length > previous.recovery.length && o.recovery.at(-1) === "served") {
        ...
        if (steps[0].choice % 4 === 1 && operationClassifier === 3 && recoveryCause === "source-error") recorder.credit("instance-allow-recovers-ordinary-error");
        if (steps[0].choice % 4 === 2 && operationClassifier === 0) recorder.credit("operation-allow-overrides-instance-denial");
      }
      if (o.recovery.length > previous.recovery.length && o.recovery.at(-1) === "served" && classifier === 3) {
        recorder.credit(`default-recovers-${recoveryCause}`);
      }
      if (step.action === "releaseLoad" && o.recovery.at(-1) === "deserialization_error" && o.calls.some((c, i) => c === 4 && previous.calls[i] === 0)) recorder.credit("recovery-failure-preserves-timeout");
