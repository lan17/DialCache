...
| C42 | Classifier precedence is operation → instance → timeout-only default, with replacement rather than union | S: recovery override scenarios, `default classifier rejects ordinary errors`, `default classifier accepts a timeout propagated by source`, `explicit denial replaces default timeout recovery`; G: recovery (operation override/default; instance precedence remains scenarios) |
