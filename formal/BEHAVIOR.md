...
The effects model fixes tracked remote-only policy to 60 seconds, separate 10 ms read and source deadlines, and held read/decode/serialize/write effects. It bounds each trace to eight callers and eight sources; successful values are 1. Model monotonic time and application wall time are separate. No local cache, recovery, shadow work, or physical expiry is enabled in this profile.

| Quint action | Driver input |
| --- | --- |
| `init` | Fresh fixture; hold reads, loads, dumps, and writes; observe adapter cancellation requests |
| `beginCall` | `begin` |
...
| --- | --- |
| Recovery `beginCall` | `begin.recovery`: choice 0=allow, 1=deny, 2=error, 3=inherit the timeout-only default |
