...
```json
{
  ...
  "maintenance": [], "loads": 0, "dumps": 0, "policyCalls": 0,
  "writeTtls": [], "shadow": [], "recovery": []
}
```

Every scenario/trace gets a fresh cache instance and empty Redis environment. State persists between its steps. The TypeScript implementation is [`test/formal/behavior-driver.ts`](../test/formal/behavior-driver.ts), used by both replay tests. No production APIs, private cache maps, or flight mutations are needed.
...
Other fixture fields are `tracked` (default false), `fallbackTimeoutMs` (default 10, null disables), `readTimeoutMs` (default 50), `localMaxSize` (default 10,000), `shadowMaxInFlight` (default 1), and `recovery` (`allow`, `deny`, or default timeout-only classification). Shadow/recovery hooks record terminal diagnostic outcomes; other metrics are outside this profile. Compression writes are disabled; envelope interoperability is covered separately by protocol vectors.

Keys use namespace `urn`, key type `id`, use case `Behavior`, and ID `1` unless an input provides `key`. The test serializer writes decimal JSON numbers and reads them back. An omitted input value denotes an undefined value, serialized as the literal text `undefined`. This custom serializer deliberately tests cached undefined without prescribing a language's JSON API. Returned undefined is represented by the string `"undefined"` in observations; other returned values are numbers.

Wall time starts at `2026-09-08T12:00:00Z`; monotonic time starts at zero. Time advances only through `advance`. Local entries use monotonic age; Redis frames/physical expiry and watermarks use wall time. Tracked reads atomically acquire a value and watermark. Watermarks remain available through the trace. A native write's frame timestamp is captured when the adapter receives it, before any held transport completion.

## Inputs and completion boundaries

| Input | Meaning |
| --- | --- |
| `begin` | Start a public call and assign its ID by invocation order. Default: a new enabled scope lasting through this call. Optional `key`, `scope`, `outside: true`, or `disabled: true`. Return control when runnable work blocks or the call settles. |
| `resolve` | Settle the specified external `loader` with `value`, or undefined if omitted. Loader IDs are actual invocation order, distinct from caller IDs. |
| `reject` | Reject `loader` with that loader's unique source error. |
...
| `seed` | Environment stores a frame for `key`: `value`, `ageMs` (default 0; negative is future), optional physical `ttlMs` (default 60,000). `frameHex` instead supplies exact raw bytes. This is external setup, not a DialCache write. |
...
| `policy` | Replace the runtime overlay with `value`. Existing entries and already accepted invocation snapshots retain their contracts. |
...
| `openScope` | Open context `id`, optionally nested inside `parent`. `disabled: true` opens a disabled context; otherwise it calls enable. Save its execution context for later inputs. |
...
1. Implement the protocol/key/normalization/envelope vectors.
