...
| `shadow` | Shadow jobs (C47 to C54): the diagnostic work a served remote hit or a ramped-down miss admits beside the caller, in one per-instance registry of bounded capacity shared by both kinds (`Jobs`: the jobs, their budgets and the sources they run for), composed by two shapes: the atomic shape (`Shadowed`, `remote_writes::Writing` with the jobs and their deadlines) judges the hit and the policy in one step at release, the held shape (`HeldShadowed`, `remote_io::Held` with the jobs, their deadlines and the selections captured for pending flights) dispatches the caller's read before the frame is judged. The policy in force for a call is `Shadowing`: the cohort selection its reply resolved (per call, C18) with the instance's `Limits` (capacity and job budget). Atomic admission beside the release judged once (`admitJobs` from `serving::layeredRelease`; `release` and `begin` compose it after `remote_writes::releaseJudged`): a served job over a detached source (`admitServed`, stated once for both shapes: `serving::startDetachedSource` with the payload the hit served as its C0), a dark job over the caller's own source reading its C0 from the remote layer itself (the read counted; the payload when the frame is fresh for the reply's freshness, nothing otherwise), a dark C0 read that fails ending the admitted job at once, `redis_error`, the read counted and nothing filled; nothing for an ordinary serving miss, a request or local hit, a key the ramp does not select or a bypass reply (the caller's scope closed before its release), `dropped` for a live job of the same identity or a full instance (`busy`, C47); the fill authority captured at admission through `serving::authority` (the retention the reply resolved to, capped for a tracked write, and the fence the C0 read observed when it found no visible frame, C18/C58); settlement of the job's source (`settle` after `remote_writes::settle`, `settleJob`: `source_error`; a fill through `remote_writes::holdDump` when there is no C0 and the captured fence still allows the write, judged before serializing as the implementations check it before encoding (`fill_fenced` at settlement otherwise, no dump), never a request or local publication; else the decode counted and `match`, or a confirmation read counted and its verdict, `confirmation`, the one judgment for both shapes: `mismatch` when the C0 bytes are still current, `superseded` otherwise, `confirmation_error` when that read fails); the held fill's releases (`releaseDump`, `releaseWrite`: `filled`, `fill_fenced`, `fill_error`; a dump released after the budget dispatches no write while a write released after it still stores, C53; a job's fill completes nobody). Held admission (`beginHeld` and `releaseHeld` over `remote_io::begin` and `release`): the selection is captured for the flight whose read the step dispatched (`capture`, a pending-only list of flights) and consulted when the hit is judged, at the decode's settlement (`settleLoad`: a fresh frame decoded completes the flight with a served hit, which admits the job through `admitServed` under the limits read then and maps its detached source to the drivers' next loader ordinal; a failed or recovery decode admits nothing), or forgotten at the read's settlement when it holds no decode (`settleRead`); the job's own decode and confirmation read are held over its source in `remote_io`'s lists under the drivers' effect indices (a held effect over a source with a job decoding or confirming is that job's): the source settled holds the decode (`settleLoader`; `source_error` for a failed source), the decode settled says `match` or holds the confirmation read under the read budget (`deserialization_error` when the decode fails), the read settled says the verdict; a timed-out job ends silently at whichever step finds it so, keeping its slot until then; a dark job's held fill is not stated, since no scheduled profile holds one. Every job's budget is kept by `deadlines` (kind `deadlines::JOB`: registered in `admit`, forgotten in `finishJob`, delivered by `advance` through `expireJob`: timed out and `timeout` said once, a dark job still waiting abandons the wait and its slot while the caller's source runs on unbounded, a served job keeps its detached source and slot until it settles, no later label follows); cohort selection stays the profile's decoder over `cohort_boundaries`; two consumers, shadow-layers over the atomic shape and admission over the held one | `begin`, `release`, `admitJobs`, `admitServed`, `admit`, `dropped`, `busy`, `settle`, `settleJob`, `confirmation`, `finishJob`, `releaseDump`, `releaseWrite`, `expireJob`, `advance`, `beginHeld`, `releaseHeld`, `settleRead`, `settleLoad`, `settleLoader`, `live`, `liveJobs`, `duplicate`, `full`, `jobsOf`, `sourceOf` |
| `diagnostics` | The diagnostics channel (`d`, a row-polymorphic `Channel[q]` so a profile whose drivers compare more fields carries them beside the four every descriptor parses): the singleflight a caller coalesced into and the layer a failed source is attributed to, as diagnosed variants of the request-only traversal; and, as diagnosed variants of the budgeted held lifecycle (`DiagnosedHeld`) for a profile whose replies are immediate and whose reads settle at admission, the singleflight a follower coalesced into (`request_local`, or the process registry's `PROCESS_SCOPE`), the layer a flight is attributed to when it starts (`REMOTE_LAYER` for one that dispatched a read, whatever the read observed; the request memo or the shared layers' fallback for a source started without the remote layer; none for a bypass source started at admission), the fallback error recorded against that layer when the flight's failure is recorded (a late result that only drains records nothing) and the recovery age sampled when a recovery decode serves (`recordAge`, C57); each held variant judges the release once, through `deadlines::releaseJudged` | `admitRequest`, `releaseRequest`, `settleRequest`, `beginRead`, `settleLoader`, `expireLoader`, `advance`, `settleLoad` |
...
served job ending silently while its slot stays owned)
