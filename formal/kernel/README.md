...
| `remote_frames` | Remote frames stamped on the wall clock and retained until an instant on the monotonic clock (`cache_rules.deadlinePendingAt`), served while retained and fresh for the reply's freshness (`cache_rules.freshAgeAllowed`, which rejects a stamp from after a wall rollback); per-entity watermarks and fences on the wall clock (`cache_rules.fenceAllows`) | `seedFrame`, `raiseWatermark`, `readableFrame`, `missFence`, `writeAllowed`, `retained`, `fresh` |
...
| `clock` | Elapsed time on the monotonic clock; a wall clock beside it for profiles whose remote frames carry wall-clock stamps, moving together on `elapse` and shifted alone on a rollback | `advance`, `elapse`, `shiftWall` |
...
| `serving` | Admission, traversal order (`decide`), ownership precedence, publication and refill authority with the TTLs a reply resolved to (`ttlsOf` for a layout's), the remote adapter's read, dump and write faults along a refill, scope closure, maintenance; the layered shape and its local and request-only projections | `admit`, `release`, `begin`, `settleWith`, `settle`, `admitLocal`, `releaseLocal`, `settleLocal`, `admitRequest`, `releaseRequest`, `settleRequest`, `closeScope`, `invalidate`, `ttlsOf` |
...
enabled layers and whether the call coalesces) with, for the layered shape, the
`layer_policy::Ttls` the reply carries (the local insertion TTL, the remote
frame's freshness and the retention a refill is written with; `Serving::ttlsOf`
is a layout's, for a profile whose replies carry none). The serving transitions
never decode a profile's policy field: `layer_policy::resolution(policy, remote)`
is the immediate reply from the drivers' layer policy codes 0 to 5, which the
layers wrapper passes to `begin` with the layout's TTLs;
`runtime_policy::resolve(state, samples)` resolves the runtime-boundaries
drivers' codes 0 to 21 against the instance's configured `Baseline`, which its
wrapper passes to `release` the same way; `BYPASS` is the reply of a call that
uses no layer and neither registry. Each profile owns its policy field with one
meaning. A resolution already reflects remote availability: the traversal reads
the remote layer whenever the resolution enables it, so a profile without
remote storage resolves `remote` to false (as `resolution` and
`runtime_policy::resolve` do).
...
inject no faults holds them false. `settleWith` settles a source with the TTLs
its own reply resolved to; `settle` is that with the layout's.
...
A composed profile's wall clock must start above zero: `cache_rules.fenceAllows`
is strict and an untracked flight's fence is 0, so a profile that initializes
`wall` to 0 refuses every remote refill without any other symptom. A profile
without a wall clock of its own initializes `wall` with `now` and advances both
through `Clock::elapse`.
...
action startCall(choice: int): bool = all {
  ...
  s' = Flights::recordIdentity(Serving::begin(s, LAYOUT, call(choice), resolution(s.policy, s.remoteAvailable), Serving::ttlsOf(LAYOUT)), call(choice)),
...
composes around `begin`; both clocks belong to `clock`, the frames with their
stamps, retention and watermarks to `remote_frames`, and the fault switches are
environment inputs the wrapper sets by record update and the traversal reads.
Composing `serving`
