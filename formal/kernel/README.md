...
fill and the buffered invalidation). It keeps
...
owes this):
`effects.mjs`, `layers.mjs`, and the scope and layers rules of `runtime.mjs`;
...
replay to the composed shadow layout the same way.
...
public-only classifier bound through the fidelity scaffold.
