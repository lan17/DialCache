...
operator is passed only inside the kernel; the lint reports any operator a
profile passes to a kernel definition, whether a lambda literal, a parametrized
profile definition passed by name, a let-bound lambda or a helper's operator
parameter forwarded to the call, so the fold cannot become a hook for rule
logic in a profile.
...
operator a profile passes to a kernel definition is reported at the call in
every spelling (a lambda literal, a parametrized profile definition passed by
name, a let-bound lambda, a helper's operator parameter forwarded to the call:
the library's folds take their operator only from kernel modules, so an
operator written in a profile is rule logic the walk cannot follow); a record literal
