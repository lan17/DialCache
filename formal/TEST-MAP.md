...
| Metrics, logs, telemetry timing | `dialcache-metrics`, `datadog`, `prometheus`, `shadow-log-json` | Shadow admission hook; effects observer isolation | Effects: observer failure during hit/publication/source failure; recovery/shadow: ages and conditional warnings | Categories, attribution, callback counts, ages, phase durations, sizes, conditional mismatch warnings | — | Generated diagnostic values remain narrower; exporter schemas and native JSON formatting are binding/integration obligations |
...
- `formal/generate-traces.sh` exports 32 core, 512 pending-effect, 256 scope, 512 recovery, 256 policy, 512 dark-shadow, 128 served-hit admission, and 512 layer-composition ITF traces. Three replay test files execute every action through public calls. Effects and feature CI require named actions plus explicit race/outcome witnesses, rather than relying on trace count alone.
...
| 2,720 configured generated traces | 68.53% | 68.00% | 80.80% | 79.21% |
| 229 scenarios + 102 protocol cases + 4 schema/audit checks | 72.54% | 72.50% | 82.72% | 79.21% |
| Generated + portable (3,055 positive tests) | 74.36% | 76.12% | 83.76% | 81.90% |

Generated library execution is 68.53% of lines / 68.00% of branches; engine execution is 80.80% / 79.21%. Combined generated/fixed execution reaches 74.36% library lines / 76.12% branches and 83.76% engine lines / 81.90% branches. Moving already-covered boundary rules into nondeterministic generated schedules improves reusable behavioral testing without necessarily reaching a new code branch. The formal corpus still misses 219 outcomes reached by ordinary tests (58 in the main engine), and reaches five outcomes absent from those tests. These counts describe execution paths, not bugs or semantic obligations.
