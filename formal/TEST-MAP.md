...
| Process-local persistence, TTL, LRU | `dialcache-local`, `dialcache-config-ramp` | Core/policy: insertion identity and TTL history | Core: repeated calls | Exact TTL boundary, LRU capacity, preserved insertion TTL/ramp changes | — | Rollback, zero capacity, and cross-operation capacity have portable scenarios; no LRU model |
