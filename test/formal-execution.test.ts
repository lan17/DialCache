...
describe("formal execution schedule", () => {
  it("accounts for all models, selected invariants, regressions, generated traces and challenges without Quint", () => {
    expect(validate(manifest())).toEqual({ models: 32, libraries: 26, profiles: 15, invariants: 221, regressions: 431,
      generatedTraces: 5280, exportedRegressionTraces: 267, vectorModels: 4, generatedVectors: 1631,
  ...
  it("requires every challenge to carry a reproducer or sit in the reported backlog, and nothing else", () => {
    ...
    expect(withReproducer.map(challenge => challenge.id)).toEqual([
      "policy-inclusive-local-expiry", "profile-recovery-wrong-snapshot", "recovery-read-wrong-admission-policy", "independent-wrong-admission-policy", "policy-inclusive-remote-freshness", "shadow-inclusive-c0-freshness", "scope-nested-close-evicts-outer-memo", "admission-capacity-off-by-one", "independent-deadline-settles-at-start", "recovery-read-inclusive-maximum", "recovery-read-recovery-warms-local", "recovery-read-tracked-retention-uncapped", "local-clock-precise-ttl", "stale-recovery-future-candidate", "envelope-strips-unknown-zero-prefix", "source-budgets-accepts-at-deadline-equality", "policy-hit-before-join", "policy-join-ignores-coalesce", "source-budgets-settlement-never-replaces-local-entry", "source-budgets-failed-settlement-clears-local-entry",
    ]);
    expect(withReproducer.map(challenge => challenge.reproducer!.kind)).toEqual([
      "exported-regression", "exported-regression", "exported-regression", "exported-regression", "exported-regression", "exported-regression", "exported-regression", "exported-regression", "exported-regression", "exported-regression", "exported-regression", "exported-regression", "exported-regression", "exported-regression", "model-run", "exported-regression", "exported-regression", "exported-regression", "exported-regression", "exported-regression",
    ]);
  ...
  it("maps every challenge to a native mutant in the catalog or an enumerated explanation, and freezes the backlog", () => {
    ...
    expect(summary.nativeMutants).toEqual({ mapped: 1, unobservable: 2, modelOnly: 68, backlog: 2 });
  ...
  it("validates reproducer kinds, cited models, declared checkpoints, profile partitions and model-only scope", () => {
    ...
    expect(validate(exported(r => { r.failure = "s.o.calls==List( DEADLINE_ERROR,CALL_PENDING )\n  and s.o.loaders == 2"; })).reproducers).toBe(20);
    expect(validate(exported(r => { r.failure = "s.o.calls == List(CALL_PENDING)"; })).reproducers).toBe(20);
    ...
    expect(validate(exported(r => { r.profiles = ["source-budgets", "effects"]; delete r.exclusions.effects; r.exclusions.independent = "Its sources settle only through explicit deadlines."; })).reproducers).toBe(20);
    ...
    expect(validate(shared(r => { delete r.exclusions.layers; r.profiles.push("layers"); })).reproducers).toBe(20);
    ...
    expect(validate(patching)).toMatchObject({ reproducers: 21, reproducerBacklog: 52 });
