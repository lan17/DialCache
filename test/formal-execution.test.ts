...
describe("formal execution schedule", () => {
  ...
  it("validates reproducer kinds, cited models, declared checkpoints, profile partitions and model-only scope", () => {
    ...
    expect(validate(exported(r => { r.profiles = ["source-budgets", "effects", "shadow-layers", "recovery", "admission", "shadow", "dark-layers"]; delete r.exclusions.effects; r.exclusions.independent = "Its sources settle only through explicit deadlines."; })).reproducers).toBe(liveReproducers());
