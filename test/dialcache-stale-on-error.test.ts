...
describe("DialCache stale-on-error recovery", () => {
  ...
  it("rereads a logical miss at the maximum age and serves it without publication", async () => {
  ...
  it("keeps feature-off writes at the fresh TTL and never performs a recovery read", async () => {
    ...
    expect(redis.readRequests.map(({ maxAgeMs }) => maxAgeMs)).toEqual([1_000, 1_000]);
  ...
  it("retains a tracked watermark through the maximum age plus its existing margin", async () => {
    const useCase = "StaleRecoveryTrackedRetention";
    const redis = new RecordingRedis();
    ...
    expect(redis.ttlMs(redisValueKey(useCase, "123", true))).toBe(10_000);
    expect(redis.ttlMs(watermarkKey())).toBe(70_000);
    expect(redis.readRequests).toEqual([
      expect.objectContaining({ maxAgeMs: 1_000, watermarkKey: watermarkKey() }),
    ]);
  });
  ...
  it("preserves the source rejection when the recovery read fails", async () => {
    const useCase = "StaleRecoveryReadError";
    const redis = new RecordingRedis();
    const sourceError = Object.freeze({ code: "SOURCE_UNAVAILABLE" });
    const { metrics, staleRecovery } = recordingMetrics();
    const dialcache = new DialCache({
  ...
  it("gives recovery an independent read deadline and preserves the source rejection on timeout", async () => {
    const useCase = "StaleRecoveryReadTimeout";
    const redis = new HangingReadRedis(2);
    const sourceError = Object.freeze({ code: "SOURCE_UNAVAILABLE" });
    const { metrics, staleRecovery } = recordingMetrics();
    const dialcache = new DialCache({
    ...
    expect(redis.readRequests).toHaveLength(2);
    expect(redis.readContexts[1]?.signal.aborted).toBe(false);
  ...
  it("rechecks tracked invalidation after the source attempt and blocks recovery", async () => {
  ...
  it("does not resurrect or extend a frame after raising the runtime recovery maximum", async () => {
    ...
    expect(rejectionReason(settled!)).toBe(sourceError);
    expect(redis.readRequests.map(({ maxAgeMs }) => maxAgeMs)).toEqual([1_000, 1_000, 10_000]);
    expect(redis.setCalls).toBe(1);
  });
  ...
  it("coalesces recovery and does not populate process-local cache", async () => {
    ...
    const localCache = (dialcache as unknown as {
      readonly localCache: { put: (...args: unknown[]) => Promise<void> };
    }).localCache;
    ...
    expect(redis.readRequests).toHaveLength(4);
    expect(localPut).not.toHaveBeenCalled();
  });
  ...
  it("contains a corrupt compression envelope and preserves the source rejection", async () => {
    ...
    const { metrics, staleRecovery } = recordingMetrics();
    const dialcache = new DialCache({ redis: { client: redis, readTimeoutMs: 1_000 }, metrics });
    const getUser = dialcache.cached(async (): Promise<{ readonly id: string }> => {
      throw sourceError;
    }, {
      keyType: "user_id",
      useCase,
  ...
  it("runs independent stale recovery chains when coalescing is disabled", async () => {
    ...
    expect(source).toHaveBeenCalledTimes(2);
    expect(redis.readRequests.map(({ maxAgeMs }) => maxAgeMs)).toEqual([1_000, 1_000, 10_000, 10_000]);
