...
function tsReport(directory: string, failure?: string) {
  ...
  return { success: !failure, numFailedTests: failure ? 1 : 0, numPassedTests: failure ? 1 : 2, numTotalTests: 2,
    ...
    testResults: [{ name: `${directory}/typescript/test/formal-features.test.ts`, status: failure ? "failed" : "passed",
...
function savedFixture(directory: string, languages: readonly unknown[] = ["typescript", "go", "rust"], results: readonly unknown[] = languages) {
  ...
  const sources: Record<string, string> = {
    ...
    "typescript/package.json": '{"name":"dialcache"}\n',
  ...
  for (const path of ["package.json", "typescript/package.json", "pnpm-workspace.yaml", "pnpm-lock.yaml"]) writeFileSync(join(directory, path), sources[path]!);
