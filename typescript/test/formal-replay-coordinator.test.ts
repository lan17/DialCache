...
describe("recording divergent histories", () => {
  ...
  it.each([false, true])("writes terminal JSONL evidence out of band (failure=%s)", failed => {
    ...
    try {
      ...
      const process = spawnSync(globalThis.process.execPath, [fileURLToPath(new URL("../../formal/replay/coordinator.mjs", import.meta.url))], {
...
describe("shared replay transport and source closure", () => {
  ...
  it("requires the exact shared source inventory before recording witness evidence", () => {
    ...
    expect(replaySources(fileURLToPath(new URL("../../", import.meta.url)))).toContain("formal/replay/coordinator.mjs");
    expect(replaySources(fileURLToPath(new URL("../../", import.meta.url)))).toContain("formal/replay/settlement.mjs");
  });
});
