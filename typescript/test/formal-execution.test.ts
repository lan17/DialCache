...
describe("formal execution schedule", () => {
  ...
  it("maps every challenge to a native mutant in the catalog or an enumerated explanation", () => {
    ...
    const explained = () => {
      ...
      for (const challenge of m.challenges) {
        challenge.nativeMutants = { kind: "model-only", text: "Fixture: no line in typescript/src/dialcache.ts carries this bookkeeping." };
      }
      return m;
    };
    ...
    expect(withNative("policy-inclusive-local-expiry", { kind: "model-only", text: `typescript/src/dialcache.ts ${"x".repeat(900)}` })).toThrow(/nativeMutants text exceeds 900 characters/);
    ...
    expect(withNative("policy-inclusive-local-expiry", { kind: "unobservable", text: "The read in typescript/src/context.ts checks closure again." })().nativeMutants).toMatchObject({ unobservable: 1 });
    ...
    const unobservable = (text: string): NativeMutants => ({ kind: "unobservable", text: `${text} The read in typescript/src/context.ts checks closure again.` });
  ...
  it("validates the unified mutant catalog and anchors every edit in the port text", () => {
    ...
    const entry = (id: string, overrides: { case?: string; description?: string; rationale?: string; typescript?: Section | null; go?: Section | null; extra?: Record<string, unknown> } = {}) => ({
      id, case: overrides.case ?? "C45.maximum-age-exclusive", description: overrides.description ?? "d", rationale: overrides.rationale ?? "Edits typescript/src/a.ts and go/a.go.",
      ...(overrides.typescript === null ? {} : { typescript: { edits: [{ path: "typescript/src/a.ts", before: "alpha", after: "beta" }], requiredDetections: ["generated", "portable"], ...overrides.typescript } }),
    ...
    expect([...anchors([sequential])().keys()].sort()).toEqual(["go/a.go", "typescript/src/a.ts"]);
    expect(anchors([entry("M01", { typescript: { edits: [{ path: "typescript/src/a.ts", before: "omega", after: "beta" }] } })])).toThrow(/Mutant anchor drift: M01: anchor must match exactly once in typescript\/src\/a.ts; review the TypeScript port or the catalog/);
    expect(anchors([entry("M01", { typescript: { edits: [{ path: "typescript/src/twice.ts", before: "alpha", after: "beta" }] } })])).toThrow(/Mutant anchor drift: M01: anchor must match exactly once in typescript\/src\/twice.ts/);
  ...
  it("keeps vector artifacts separate from profile histories and validates their provenance boundary", () => {
    for (const changed of [
      ...
      (v: NonNullable<Manifest["models"][number]["vectorExport"]>) => { v.sources.push("typescript/src/key.ts"); },
    ]) {
      const invalid = manifest();
