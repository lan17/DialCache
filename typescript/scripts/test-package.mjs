...
void (async () => {
  ...
  const ports = JSON.parse(await readFile(join(repositoryRoot, "docs/ports.json"), "utf8"));
  const documentationSource = ports.find((port) => port.id === "typescript").example;
  await writeFile(join(workspace, "docs.mts"), await readFile(join(repositoryRoot, documentationSource), "utf8"));
