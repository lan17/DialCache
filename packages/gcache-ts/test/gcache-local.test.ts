...
describe("GCache local-only MVP", () => {
  ...
  it("does not leak enabled context across parallel async flows", async () => {
    ...
    const getValue = gcache.define(
      ...
      (tenantId: string) => ({
        cacheKey: tenantId,
        loader: async () => ({ tenantId, calls: ++calls }),
      }),
    );
  ...
  it("keeps delimiter-containing ids and args in distinct local cache keys", async () => {
    ...
    const search = gcache.define(
      ...
      (userId: string, filter?: string) => ({
        cacheKey: { id: userId, args: { filter } },
        loader: async () => ({ userId, filter, calls: ++calls }),
      }),
    );
  ...
  it("uses sorted explicit args as part of the cache key", async () => {
    ...
    const search = gcache.define(
      ...
      (userId: string, page: number, filter: string) => ({
        ...
        loader: async () => ({ userId, page, filter, calls: ++calls }),
      }),
    );
