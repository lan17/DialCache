...
The GLIDE adapter uses GLIDE's native `Script` lifecycle and byte decoder for scalar operations and, when the supplied runtime exposes the necessary capabilities, its native standalone or cluster batch support for batch invalidation. The native batch path uses the invalidation script's hash with `EVALSHA`; after a cold `SCRIPT FLUSH`, a script-cache miss retries the whole idempotent batch once with `EVAL`, and subsequent warm batches return to one `EVALSHA` execution. A runtime whose `Script` handle does not expose `getHash()` uses `EVAL` directly. GLIDE Cluster batches enable GLIDE's server-error and connection-error retries, so a batch may be replayed; monotonic invalidation makes that replay safe. Standalone batches do not opt into those retry flags.
