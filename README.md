...
An invalid runtime `shadow.ramp` does not affect the cache result or disable an otherwise valid Redis policy. If normal traversal reaches an otherwise shadow-eligible Redis path, DialCache skips shadow work and records a `config_resolution` error. Invalid runtime shadow logging leaves likewise preserve the cache result, Redis policy, shadow result, and shadow metric: an invalid `logMismatches` disables the warning, while an invalid `logMismatchDetails` downgrades it to the metadata-only warning. DialCache validates these diagnostic leaves only after the metrics hook, exact-key cohort, and capacity gates admit shadow work, and collapses multiple invalid logging leaves into one remote `config_resolution` error per admitted resolution.
