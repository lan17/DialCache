...
After validating, building, and package-testing the release artifact, the workflow stamps `package.json` and pushes a `chore(release): <version> [skip ci]` commit to `main`. An isolated publish run then verifies that exact commit is still current `main`, reruns the package checks, and asks Semantic Release to create the matching Git tag, publish the public npm package with provenance, and publish the GitHub release. The version in `package.json`, the release tag, npm, and the GitHub release therefore agree.

The protected `main` ruleset must list the GitHub Actions app as an always-allowed bypass actor so the manual workflow can push only this release commit. Repository workflows remain least-privileged; the release workflow is the only workflow granted `contents: write`.
